...
import {
  ...
  ChevronDown, User, School, CreditCard,
} from "lucide-react";
...
function PropinasView({ token, propinas, alunos, onOpenGerarPropina, onOpenGerarRef }: {
    ...
    <div className="p-6 lg:p-8 flex-1 overflow-y-auto">
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        <div className="flex gap-1 bg-slate-100 p-1 rounded-xl">
          {[{k:"todos",l:"Todas"},{k:"pendente",l:"Pendentes"},{k:"pago",l:"Pagas"}].map(({k,l}) => (
            <button key={k} onClick={() => setFilterStatus(k as any)}
              className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all ${filterStatus===k?"bg-white text-slate-900 shadow-sm":"text-slate-500 hover:text-slate-700"}`}>{l}</button>
          ))}
        </div>
      ...
      {filtered.length === 0 ? (
        ...
        <Card className="p-0 overflow-hidden">
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 text-slate-500 text-xs font-semibold uppercase border-b border-slate-100">
              <tr><th className="px-5 py-3">Aluno</th><th className="px-5 py-3">Turma</th><th className="px-5 py-3">Período</th><th className="px-5 py-3">Valor</th><th className="px-5 py-3">Estado</th><th className="px-5 py-3">Referência</th><th className="px-5 py-3">Validade</th></tr>
            </thead>
            <tbody className="divide-y divide-slate-100 bg-white">
              {filtered.map((p, i) => (
                <tr key={i} className="hover:bg-slate-50/50">
                  ...
                  <td className="px-5 py-3 text-xs text-slate-500">{p.ref_validade ? fmtDate(p.ref_validade) : "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
